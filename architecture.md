...
application
  ...
  只能依赖 core；不定义公开 schema 或 DTO。

cyreneAI.bootstrap
  默认 composition root：把 core、application 和 infra 装成可运行 runtime。

server
  HTTP/API 层：读取环境配置，创建 HTTP/API 入口；只依赖 core、默认 composition root、application runtime 和 server 自身模块。
```
...
对应 import 边界图如下。这张图只表达模块级 import 方向，不表达运行时调用顺序；箭头方向表示“谁可以 import 谁”。

```mermaid
flowchart LR
  core["core\nschema / protocol / manager / errors"]
  app["application\nruntime / orchestrators"]
  infra["infra\nexternal system implementations"]
  infra_catalog["infra/provider_catalog\n*_info.py only"]
  infra_adapters["infra/adapters\nSDK clients / stores / executors"]
  infra_bootstrap["infra/bootstrap\nregistrations"]
  public_adapters["adapters\npublic adapter facade"]
  public_api["api\nplugin SDK facade"]
  ...
  infra_catalog --> core
  infra_adapters --> core
  infra_bootstrap --> core
  infra_bootstrap --> infra_catalog
  infra_bootstrap --> infra_adapters

  public_api --> core
  public_adapters --> core
  public_adapters --> infra_adapters
  ...
  classDef coreLayer fill:#eef9f0,stroke:#2f8f46,color:#111;
  classDef appLayer fill:#e8f1ff,stroke:#3267c8,color:#111;
  classDef infraLayer fill:#fff4e6,stroke:#d18419,color:#111;
  classDef publicLayer fill:#f5ecff,stroke:#7b45c6,color:#111;
  classDef serverLayer fill:#f7f7f7,stroke:#777,color:#111;

  class core coreLayer;
  class app appLayer;
  class infra,infra_catalog,infra_adapters,infra_bootstrap infraLayer;
  class public_adapters,public_api,root publicLayer;
...
```text
core
  禁止 import openai、httpx、dotenv、anthropic、google 等外部 SDK。
  禁止读取 .env 或环境变量。
  禁止创建外部 SDK client。
  禁止写 provider 专属实现规则。
  禁止 import cyreneAI.infra、cyreneAI.application、cyreneAI.server、cyreneAI.adapters。

application
  禁止 import cyreneAI.infra、cyreneAI.server。
  禁止 import openai、httpx、anthropic、google 等外部 SDK。
  禁止读取 .env 或环境变量。
  禁止创建外部 SDK client、database store、filesystem loader。
  禁止注册 provider adapter。
  禁止处理 provider 专属协议差异。
  禁止定义 CyreneAISchema 派生类。
  禁止定义公开 @dataclass DTO。

infra
  禁止 import cyreneAI.application、cyreneAI.server。
  外部 SDK 只能在 adapter 实现内出现。

server
  禁止 import cyreneAI.infra、cyreneAI.adapters。
  禁止 import cyreneAI.api；插件 SDK API 只给插件作者使用。
```

`core/schema` 是唯一 schema 定义目录。任何继承 `CyreneAISchema` 的类型只能放在 `src/cyreneAI/core/schema/`；应用层 request/result schema 也属于 core schema，例如 `ApplicationChatRequest`、`ApplicationChatResult`、`ApplicationBotRequest`、`ApplicationRAGChatRequest`。`application` 只能 import 并使用这些类型，不能用 `@dataclass` 绕过 schema 边界定义公开 DTO；只有私有算法 helper 可以使用私有 dataclass，例如 `_ParagraphSpan`。公开 runtime 容器需要进测试白名单。
...
infra/provider_catalog/{provider}_info.py
  只声明 provider 注册信息。
  只能 import core/schema。
  禁止 import 外部 SDK。
  目录内只允许 __init__.py 和 *_info.py。

infra/adapters/providers/{provider}/
  放外部系统适配实现。
  允许 import 外部 SDK、创建 client、调用外部 API。
  负责请求映射、响应映射、异常翻译和 provider 协议差异。
  provider 目录只允许 __init__.py、builder.py、errors.py、instance.py、mapper.py。

infra/bootstrap/registrations/{provider}.py
  只负责把 provider info 和 adapter builder 接到 registry/factory。
  只能依赖 core、infra/provider_catalog、infra/adapters 和 infra/bootstrap。
  禁止 import application、server 或外部 SDK。
```

默认 composition root 只能是 `src/cyreneAI/bootstrap.py`。它可以同时 import `core`、`application` 和 `infra`，因为它负责把默认运行时总装起来。除此之外，`application` 不装配 infra 具体实现，`infra` 不装配 application runtime，`server` 不手写 provider adapter 注册细节，只调用 composition root。
...
```text
test_infra_provider_catalog_only_contains_info_files
  provider_catalog 只允许 __init__.py 和 *_info.py。
...
test_provider_catalog_imports_only_core_schema
  provider_catalog 只 import core/schema，且不 import 外部 SDK。
...
test_provider_adapter_directories_have_expected_files
  ...
  infra/adapters/providers 顶层额外只允许 __init__.py 和 model_mapper.py。

test_provider_adapters_do_not_import_application_or_server
  provider adapters 不 import application 或 server。

test_infra_does_not_import_application_or_server
  infra 整体不 import application 或 server。

test_bootstrap_registrations_only_wire_core_catalog_and_adapters
  bootstrap registrations 只装配 core、provider_catalog、adapters、infra bootstrap。

test_application_does_not_import_infra_or_server
  application 不 import infra 或 server。

test_server_does_not_import_infra_or_adapters
  server 不 import infra 或 cyreneAI.adapters。

test_server_does_not_import_plugin_sdk_api
  server 不 import cyreneAI.api。
...
test_application_top_level_directories_are_intentional
...
infra/adapters/providers/{provider}/
infra/bootstrap/registrations/{provider}.py
tests/test_{provider}_*.py
tests/test_provider_registrations.py
tests/test_infra_provider_boundaries.py
```
...
新增 application request/result schema：

```text
core/schema/application.py 或 core/schema/{domain}.py
application 只 import 和使用
tests/test_*_schema.py
tests/test_infra_provider_boundaries.py
```

新增 application runtime 容器字段：
...
如果需要公开 `@dataclass` runtime 容器，必须同步更新 `tests/test_infra_provider_boundaries.py` 的白名单；普通公开 DTO 不允许放在 application。
...
server 可以读取环境变量并解析部署配置，但不能直接 import infra、`cyreneAI.adapters` 或 `cyreneAI.api`。如果配置需要创建外部实现，先把配置解析为参数，再传给 `cyreneAI.bootstrap`。
